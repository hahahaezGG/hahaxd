...
client.on('guildCreate', async guild => {
    ...
    guildhook.send(`Shard ${client.shard.id + 1} > Yeni Sunucu > Sunucu Adı: "${guild.name}", Sunucu ID: "${guild.id}", Sunucu Sahibi: "${guild.owner}", Sunucu Sahibi ID "${guild.ownerID}" > ${client.guilds.size}. sunucu`)
		const girismesaj = [
		  ...
		  '**Sohbet ve Oyun BOT** sunucunuzdaki insanlara ve size kolaylıklar saglamak amacıyla yapilmistir.',
		  'Bot Hasan tarafından geliştirilmektedir',
		  'Botumuzun özelliklerini öğrenmek için svo!yardım komutunu kullanabilirsiniz.',
		  '',
		  `**Bot Davet Linki Kaldıralabilir**`
		]
  ...
  .on('message', msg => {
	...
	.on('ready', async () => {
		...
		await client.user.setPresence({ game: { name: `svo!yenilikler | svo!yardım | SVO Bot`, type: 0 } });
